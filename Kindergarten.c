...
Garden**	readAllGardensFromFile(char* fileName,int* pGardenCount)
{
	...
	*pGardenCount = 0;
	...
	if ((fp = fopen(fileName, (fileType == TEXT)?"rt":"rb")) == NULL)
	{
...
void	readGarden(FILE* fp, Garden* pGarden)
{
	...
	}else
	{
		...
		fread(sTemp,sizeof(char),i,fp);
		//Read Kindergarten binary data
		fread(&bdata,1,1,fp);
		type = (int)(bdata & 0x3);
		pGarden->childCount = (int)(bdata >> 2);
...
void	writeGardensToFile(Garden** pGardenList, int gardenCount, char* fileName)
{
	int i;
	FILE *fp;
	if ((fp = fopen(fileName, (fileType == TEXT)?"wt":"wb")) == NULL)
	{
		printf("Cannot Open File '%s'", fileName);
		return;
	}
...
void	writeGarden(FILE* fp,const Garden* pGarden)
{
	...
	{
		i = strlen(pGarden->name) + 1;
		fwrite(&i, sizeof(int),1,fp);
		fwrite(pGarden->name,sizeof(char),i,fp);
		bdata = pGarden->type & 0x3;
		bdata |= pGarden->childCount << 2;
		fwrite(&bdata,1,1,fp);
	}
	for (i = 0; i <pGarden->childCount; i++)
...
int compareGardenByName(const void* a, const void* b)
{
	return strcmp((*(Garden**)a)->name,(*(Garden**)b)->name);
}

int compareGardenByType(const void* a, const void* b)
{
	int res;
	res = (*(Garden**)a)->type - (*(Garden**)b)->type;
	return (res != 0)? res : ((*(Garden**)a)->childCount - (*(Garden**)b)->childCount);
}
